...
from .forms import EditProfileModelForm, ChangePasswordModelForm
    ...
    def post(self, request: HttpRequest):
        ...
        if form.is_valid():
            ...
            if is_password_correct:
                ...
                form.add_error('current_password','رمز ورود فعلی وارد شده اشتباه میباشد.')
        context = {
            'form': form
        }
        return render(request, 'user_panel_module/change_password_page.html', context)
...
class MyShoppings(ListView):
    model = Order
    template_name = 'user_panel_module/shoppings_list_page.html'

    def get_queryset(self):
        queryset = super().get_queryset()
        return queryset.filter(user_id=self.request.user.id, is_paid=True)

@login_required
def my_shopping_detail(request, order_id):
    order = Order.objects.prefetch_related('orderdetail_set').filter(is_paid=True, id=order_id, user_id=request.user.id).first()

    return render(request, 'user_panel_module/shoppings_detail_list_page.html', {'order': order})
